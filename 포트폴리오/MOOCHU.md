...
<summary><b>시연 영상</b></summary> <br>
<div align="center" markdown="1">
	https://www.youtube.com/watch?v=untP2jhViK0
</div>
...
### 1. 팀원들과의 성향 차이
프로젝트를 진행할 때 굉장히 친한 팀원들과 팀을 꾸려서 진행을 했습니다. 하지만 일을 하면 할수록 일하는 스타일이 서로 맞지 않는 부분이 보였습니다. 일의 순서, 팀원과의 의사소통 문

제 등등에 의해서 갈등이 있었습니다. 너무 중요한 프로젝트라 이 분위기를 어떻게 헤쳐 나가고 어떻게 순화를 시켜야할지 고민을 많이 했던 것 같습니다. 갈등을 일으킨 팀원 2명을 따로 만

나서 이야기를 나누었고 서로의 입장을 들어본 뒤에 함께 회식 자리를 가지며 오해를 풀었고 서로를 이해를 해주는데 있어서 팀원들과 함께 중재를 했습니다. 이런 충돌이 있고 난 뒤에 팀워

크가 잘 맞고 분위기도 더 좋아졌습니다. 나중에 회사에 가게 돼서 프로젝트를 할 때 이렇게 부딪히는 일이 있으면 거기서 더 좋은 분위기를 가지기 위해 이런 부딪힘이 있는 것이고 함께 

이 상황을 헤쳐나가봐야겠다 라는 마음으로 임할 수 있을 것 같습니다.

### 2. 데이터 적재, 전처리에 대한 흥미
영화에 대한 정보들을 크롤링을 해야하는 상황이었고 제목, 출연진, 러닝타임, 장르 등등의 정보를 모았습니다. 하지만 영화 사이트마다 장르와 출연진에 대한 데이터의 정합성이 맞지 않는 

부분이 있어서 그러한 부분을 전처리를 하여 데이터를 모았습니다. 403오류가 떠서 어떻게 해야할지 고민과 많은 시도를 진행했었고 결국 403에러가 뜨지 않을 정도의 기준을 찾았고

그 기준만큼 팀원들과 나눠서 크롤링을 진행하여 데이터를 모았습니다. 이러한 일을 하면서 어떻게 해야 할지 고민하고 계획하고 생각하면서 여러가지 시도를 하며 결과를 도출했을 때

그 쾌감이 저에게 흥미를 이끌어주었습니다. 데이터와 더 친해진 느낌이었고 이 흥미를 가지고 더욱 많은 것들을 해보고 싶다는 생각이 들었습니다.
